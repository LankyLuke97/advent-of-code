...
class MinPriorityQueue
    ...
    def dup
        copied_queue = MinPriorityQueue.new
        @heap.each do |node|
            copied_queue.enqueue(node[:element], node[:priority])
        end
        copied_queue
    end
...
endDistances = []
startPos.each do |start|
    queueCopy = queue.dup
    distances[start] = 0
    queueCopy.update_priority(start, 0)

    while !queueCopy.empty?
        index = queueCopy.dequeue

        graph[index].each do |neighbour|
            alt = distances[index] + neighbour[1]
            if alt < distances[neighbour[0]]
                distances[neighbour[0]] = alt
                queueCopy.update_priority(neighbour[0], alt)
            end
        end
    end
end
