...
def part2(test=False, file_path=None):
    ...
    orig_registers = [int(i.split(':')[1].strip()) for i in inp[:3]]
    program = [int(i) for i in inp[3].split(':')[1].strip().split(',')]
    pos = 1
    a_reg = 1
    octals = []
    ...
    while True:
        registers = [a_reg, orig_registers[1], orig_registers[2]]
        pointer = 0
        out = []
        ...
        while pointer < len(program) - 1:
            ...
            pointer += 2
        if out == program: return a_reg, perf_counter() - start_time
        if out[0] == program[-pos]:
            print(f'Matches {out[0]} to program[-{pos}] with a_reg = {a_reg}')
            octals.append(a_reg)
            if pos == len(program): break
            a_reg = 8 ** pos
            print(f'Setting {a_reg}')
            pos += 1
            continue
        else: a_reg += 1
    print(octals)
    end_time = perf_counter()
    return sum(octals), end_time - start_time
