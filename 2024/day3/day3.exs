defmodule Day3 do
  ...
  def part2(path) do
    ...
    |> Enum.join()
    |> then(fn input ->
      Regex.scan(~r/mul\((\d{1,3}),(\d{1,3})\)|do\(\)|don't\(\)/, input)|> Enum.map(fn
        ["mul(" <> _ = _, x, y] -> [String.to_integer(x), String.to_integer(y)]
        [match] -> [match]
      end)
    end)
    |> Enum.reduce([0, true], fn
      [x, y], [acc, true] -> [acc + (x * y), true]
      ["do()"], [acc, false] -> [acc, true]
      ["don't()"], [acc, true] -> [acc, false]
      _, [acc, enabled] -> [acc, enabled]
    end)
    |> Enum.at(0)
  end
end
...
test = Day3.part2("day3/data/test2.txt")
